package com.liubing.framework.orm.dialect;

import java.util.*;

import org.apache.commons.lang3.StringUtils;

import com.liubing.framework.orm.dialect.impl.H2Dialect;
import com.liubing.framework.orm.dialect.impl.MySqlDialect;
import com.liubing.framework.orm.dialect.impl.OracleDialect;
import com.liubing.framework.orm.dialect.impl.SqlServerDialect;
import com.liubing.framework.orm.exception.SysError;
import com.liubing.framework.orm.exception.SystemException;


public abstract class Dialect {
    private static final Map<String, Dialect> dialect_map = new HashMap<String, Dialect>();
    private static final Set<String> iso_reservedWords = new HashSet<String>();
    ...
    public static Dialect getDialect(String name) {
        Dialect dialect = (name == null) ? null : dialect_map.get(name.toLowerCase());
        if (dialect == null) {
            throw new SystemException("Unsupport database.", SysError.CLASS_NOT_FOUND).set("dialect.name", name);
        }
        return dialect;
    }
    ...
    public String convertJavaStringToSqlType(String type, Integer length, Integer scale){
    ...
    /**
     * 将数据库无关的SubStyleType转换成具体数据库的字段名称。特殊的字段类型由子类实现。
     * @param type  数据库无关的字段名称，@see {SubStyletType}
     */
    public String asSqlType(String type, Integer length, Integer scale) {
        if (SubStyleType.UID.equals(type)) {
            ...
            return SqlType.newInstance("decimal", length, scale).toString();
        } else if (SubStyleType.CHAR.equals(type)) {
            return SqlType.newInstance("char", length, null).toString();
        } else if (SubStyleType.VARCHAR.equals(type)) {
            return SqlType.newInstance("varchar", length, null).toString();
            ...
            return SqlType.newInstance(type, length, scale).toString();
        }
    }
    ...
    private static void sql92_reservedWords() {
    ...
    private static void sql99_reservedWords() {
