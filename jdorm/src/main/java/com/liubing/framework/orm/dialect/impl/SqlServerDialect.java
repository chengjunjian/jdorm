package com.liubing.framework.orm.dialect.impl;

import com.liubing.framework.orm.dialect.Dialect;
import com.liubing.framework.orm.dialect.SqlType;
import com.liubing.framework.orm.dialect.SubStyleType;
...
public class SqlServerDialect extends Dialect {
    ...
    public String sql_pagelist(String sql, int pageIndex, int pageSize) {
        if (pageIndex == 0) {
            sql = "select top " + pageSize + " * from (" + sql + ") as temp";
        } else {
            ...
            //@formatter:off
            sql = "select * from ("
                + "  select top " + pageSize+ " row_number() over(" + sorts + ") as row, * from (" + sql + ")" 
                + ") as temp where row > " + pageSize*pageIndex;
            //@formatter:on
        }
        return sql;
    }
